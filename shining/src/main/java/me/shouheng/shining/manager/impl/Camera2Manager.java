package me.shouheng.shining.manager.impl;
...
import me.shouheng.shining.listener.CameraCloseListener;
...
import me.shouheng.shining.preview.CameraPreviewCallback;
...
public class Camera2Manager extends BaseCameraManager<String> implements ImageReader.OnImageAvailableListener {
                ...
                try {
                    cameraManager.openCamera(currentCameraId, new CameraDevice.StateCallback() {
                        ...
                        public void onDisconnected(@NonNull CameraDevice camera) {
                            SLogger.e(TAG, "Camera disconnected.");
                            camera.close();
                            cameraDevice = null;
                        ...
                        public void onError(@NonNull CameraDevice camera, int error) {
                            SLogger.e(TAG, "Camera open error : " + error);
                            camera.close();
                            cameraDevice = null;
    ...
    public void setMediaType(int mediaType) {
        SLogger.d(TAG, "setMediaType : " + mediaType + " with mediaType " + this.mediaType);
        ...
        if (isCameraOpened() && previewRequestBuilder != null) {
            ...
            if (captureSession != null) {
                try {
                    ...
                    SLogger.e(TAG, "setAutoFocus error : " + e);
                }
            } else {
                SLogger.i(TAG, "setAutoFocus captureSession is null.");
            }
        } else {
            SLogger.i(TAG, "setAutoFocus camera not open or previewRequestBuilder is null");
        }
    }
            ...
            if (succeed) {
                ...
                if (captureSession != null) {
                    try {
                        ...
                        SLogger.e(TAG, "setFlashMode error : " + e);
                    }
                } else {
                    SLogger.i(TAG, "setFlashMode captureSession is null.");
                }
            } else {
                SLogger.i(TAG, "setFlashMode failed.");
            }
        } else {
            SLogger.i(TAG, "setFlashMode camera not open or previewRequestBuilder is null");
        }
    }
            ...
            if (succeed) {
                ...
                if (captureSession != null) {
                    try {
                        ...
                        SLogger.e(TAG, "setZoom error : " + e);
                    }
                } else {
                    SLogger.i(TAG, "setZoom captureSession is null.");
                }
            } else {
                SLogger.i(TAG, "setZoom failed : setZoomInternal failed.");
            }
        } else {
            SLogger.i(TAG, "setZoom failed : camera not open.");
        }
    }
            ...
            backgroundHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (prepareVideoRecorder()) {
                        ...
                        SLogger.i(TAG, "startVideoRecord : failed when prepare video recorder.");
...
//        releaseTexture();
    ...
    public void onImageAvailable(ImageReader reader) {
        try {
            try (Image image = reader.acquireNextImage()) {
                ...
                SLogger.e(TAG, "onImageAvailable error" + e);
    ...
    private void initCameraInfo(Context context) {
        ...
        try {
            ...
            SLogger.e(TAG, "initCameraInfo error " + e);
        }
        SLogger.d(TAG, "initCameraInfo basic cost : " + (System.currentTimeMillis() - start) + " ms");
    ...
    private void prepareCameraOutputs() {
        ...
        try {
            ...
            SLogger.e(TAG, "initCameraInfo error " + ex);
            notifyCameraOpenError(new RuntimeException(ex));
        }
        SLogger.d(TAG, "initCameraInfo get map cost : " + (System.currentTimeMillis() - start) + " ms");
        ...
        SLogger.d(TAG, "prepareCameraOutputs cost : " + (System.currentTimeMillis() - start) + " ms");
    }
                        ...
                        cameraDevice.createCaptureSession(Arrays.asList(workingSurface, imageReader.getSurface()),
                                new CameraCaptureSession.StateCallback() {
                                    ...
                                    public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                                        if (isCameraOpened()) {
                                            ...
                                            try {
                                                ...
                                                SLogger.e(TAG, "createPreviewSession error " + ex);
                                                notifyCameraOpenError(ex);
                                    ...
                                    public void onConfigureFailed(@NonNull CameraCaptureSession cameraCaptureSession) {
                                        SLogger.d(TAG, "onConfigureFailed");
            ...
            try {
                ...
                captureSession.capture(captureBuilder.build(), new CameraCaptureSession.CaptureCallback() {
                    ...
                    public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                        ...
                        SLogger.d(TAG, "onCaptureCompleted: ");
                    }
                }, null);
            } catch (CameraAccessException e) {
                SLogger.e(TAG, "Error during capturing picture");
                notifyCameraCaptureFailed(e);
            }
        } else {
                    ...
                    cameraDevice.createCaptureSession(surfaces, new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                            ...
                            try {
                                ...
                                SLogger.e(TAG, "videoRecorder.start(): " + e);
    ...
    private void runPreCaptureSequence() {
        try {
            ...
            SLogger.e(TAG, "runPreCaptureSequence error " + e);
        }
    }
    ...
    private boolean setFlashModeInternal() {
        try {
            ...
            if (isFlashAvailable == null || !isFlashAvailable) {
                SLogger.i(TAG, "Flash is not available.");
                return false;
            }
            switch (flashMode) {
                case Flash.FLASH_ON:
            ...
            SLogger.e(TAG, "setFlashMode error : " + ex);
        }
        return false;
    }
    ...
    private void lockFocus() {
        try {
            ...
            SLogger.e(TAG, "lockFocus : error during focus locking");
        }
    }
    ...
    private void unlockFocus() {
        try {
            ...
            SLogger.e(TAG, "unlockFocus : error during focus unlocking");
        }
    }
    ...
    private boolean prepareVideoRecorder() {
        ...
        try {
            ...
            SLogger.e(TAG, "IllegalStateException preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        } catch (IOException error) {
            SLogger.e(TAG, "IOException preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        } catch (Throwable error) {
            SLogger.e(TAG, "Error during preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        }
    ...
    private void closePreviewSession() {
        if (captureSession != null) {
            ...
            try {
                ...
                SLogger.e(TAG, "closePreviewSession error : " + e);
            } finally {
                captureSession = null;
            }
        }
