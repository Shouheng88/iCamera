...
public class Camera1Manager extends BaseCameraManager<Integer> {
    ...
    public Camera1Manager(CameraPreview cameraPreview) {
        ...
        cameraPreview.setCameraPreviewCallback(new CameraPreviewCallback() {
            ...
            public void onAvailable(CameraPreview cameraPreview) {
                SLogger.d(TAG, "onAvailable : " + cameraPreview.isAvailable());
                if (isCameraOpened()) {
                    setupPreview();
                }
            }
    ...
    public void switchCamera(int cameraFace) {
        SLogger.d(TAG, "switchCamera : " + cameraFace);
            ...
            public void run() {
                try {
                    if (!takingPicture) {
                        ...
                        SLogger.i(TAG, "takePicture : taking picture");
                    }
                } catch (Exception ex) {
                    takingPicture = false;
                    SLogger.e(TAG, "takePicture error : " + ex);
    ...
    private void adjustCameraParameters(boolean forceCalculateSizes, boolean changeFocusMode, boolean changeFlashMode) {
        ...
        SLogger.d(TAG, "adjustCameraParameters size cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        if (changeFocusMode) {
            setFocusModeInternal(parameters);
        }
        SLogger.d(TAG, "adjustCameraParameters focus cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        if (changeFlashMode) {
            setFlashModeInternal(parameters);
        }
        SLogger.d(TAG, "adjustCameraParameters flash cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        setZoomInternal(parameters);
        SLogger.d(TAG, "adjustCameraParameters zoom cost : " + (System.currentTimeMillis() - start) + " ms");
        ...
        SLogger.d(TAG, "adjustCameraParameters restart preview cost : " + (System.currentTimeMillis() - start) + " ms");
    }
    ...
    private void setAutoFocusInternal(@NonNull android.hardware.Camera.Parameters parameters) {
        try {
            ...
            SLogger.e(TAG, "setAutoFocusInternal " + ex);
        }
    }
    ...
    private void setFlashModeInternal(@Nullable android.hardware.Camera.Parameters parameters) {
        ...
        try {
            ...
            SLogger.e(TAG, "setFlashModeInternal : " + ex);
        }
    }
